...
void ColorPicker::PlaceWindow(const HWND hwnd, const RECT rc) {
    ...
    if ((rc.left + win_width) > screen_width) {
        x = screen_width - win_width;
    } else {
        x = rc.left;
    }

    if ((rc.bottom + win_height) > screen_height) {
        y = rc.top - win_height;
    } else {
        y = rc.bottom;
    }
...
BOOL ColorPicker::OnScreenPick(WPARAM wparam) {
...
void ColorPicker::GenerateColorPaletteData(float alpha) {
    ...
    for (int i = 0; i < PALETTE_COLUMN; i++) {
        int t0 = clamp((i - RECENT_ZONE_COLUMN) * 0x11, 0, 0xFF);
...
void ColorPicker::PaletteMouseMove(const POINT p) {
    ...
    int index = max(0, round((p.x - _rect_palette.left) / PALETTE_CELL_SIZE));
    int row = max(0, round((p.y - _rect_palette.top) / PALETTE_CELL_SIZE));
...
void ColorPicker::DrawAdjustZoneHoverBox(int row, int index, bool is_hover) {
    ...
    if (is_hover) {
        ...
        if (row == _adjust_center_row) {
            ...
            DrawColorBlock(hdc, rc, color, index != 3);
        }
    }
...
void ColorPicker::AdjustZoneMouseMove(const POINT p) {
    int index = max(0, round((p.x - _rect_adjust_buttons.left) / (ADJUST_ZONE_CELL_WIDTH + 1)));
    int row = max(0, round((p.y - _rect_adjust_buttons.top) / ADJUST_ZONE_CELL_HEIGHT));

    _show_picker_cursor = (row == _adjust_center_row);
...
void ColorPicker::ShowNativeColorChooser() {
...
bool ColorPicker::hex2rgb(const char* hex_str, RGBAColor* out_color) {
    ...
    if (strtok_s(hex_copy, "0123456789ABCDEFabcdef", &next_token) != NULL) {
        return false;
    }
...
HSLAColor ColorPicker::rgb2hsl(RGBAColor rgb) {
    double r = (double)rgb.r / 255.0f;
    double g = (double)rgb.g / 255.0f;
    double b = (double)rgb.b / 255.0f;
...
RGBAColor ColorPicker::hsl2rgb(double h0, double s0, double l0, float a0) {
    double h = (int)h0 % 360 + (h0 - (int)h0);
    if (h < 0) {
        h = 360.0 + h;
    }

    double s = clamp(s0, 0.0, 1.0);
    double l = clamp(l0, 0.0, 1.0);
    float a = clamp(a0, 0.0f, 1.0f);
    ...
    if(s > 0) {
        ...
        tr = calc_color(th + (1.0 / 3.0), t1, t2);
        tg = calc_color(th, t1, t2);
        tb = calc_color(th - (1.0 / 3.0), t1, t2);
    }

    return RGBAColor(round(tr * 255.0), round(tg * 255.0), round(tb * 255.0), a);
}
