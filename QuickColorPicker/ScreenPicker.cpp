...
void ScreenPicker::Start() {
    ...
    ::SetWindowPos(_mask_window, HWND_TOPMOST, 0, 0, width, height, SWP_SHOWWINDOW);
...
void QuickColorPicker::ScreenPicker::ReduceMouseSpeed() {
    // save mouse speed
    SystemParametersInfo(SPI_GETMOUSESPEED, 0, &_mouse_speed, 0);

    // slow it down
    SystemParametersInfo(SPI_SETMOUSESPEED, 0, (LPVOID)1, SPIF_UPDATEINIFILE || SPIF_SENDCHANGE || SPIF_SENDWININICHANGE);
}

void QuickColorPicker::ScreenPicker::RestoreMouseSpeed() {
    SystemParametersInfo(SPI_SETMOUSESPEED, 0, (LPVOID)_mouse_speed, SPIF_UPDATEINIFILE || SPIF_SENDCHANGE || SPIF_SENDWININICHANGE);
}
...
void ScreenPicker::PlaceInfoWindow(int x, int y) {
    ...
    if (win_x + INFO_WINDOW_WIDTH > ::GetSystemMetrics(SM_CXVIRTUALSCREEN)) {
        win_x = x - 20 - INFO_WINDOW_WIDTH;
    }

    if (win_y + INFO_WINDOW_HEIGHT > ::GetSystemMetrics(SM_CYVIRTUALSCREEN)) {
        win_y = y - 20 - INFO_WINDOW_HEIGHT;
    }

    ::MoveWindow(_info_window, win_x, win_y, INFO_WINDOW_WIDTH, INFO_WINDOW_HEIGHT, true);
}
